...
<body>
	...
	<div class="nav">
		<ul class="userMenu">
            <li class="current"><a class="menua" href="/index.php?m=admin&c=user&a=index" <?php if($type == 1): ?>style="display:none"<?php endif; ?>>时间</a></li>
            <li><a class="menua" href="/index.php?m=admin&c=user&a=order" <?php if($type == 0): ?>style="display:none"<?php endif; ?>>查询</a></li>
            <li><a class="menua" href="/index.php?m=admin&c=user&a=change" <?php if($type == 0): ?>style="display:none"<?php endif; ?>>已预约</a></li>
	...
	<div class="section">
			...
			<div class="lshow">
				...
				<form enctype="multipart/form-data" id="addform" method="post">
					...
					<label>日期</label><input type="date" class="search" name="date" value="" id="search" min=<?php echo ($data); ?>></br>
					<label>开始时间</label><input type="time" class="search" name="stime" value="" id="search"></br>
					<label>结束时间</label><input type="time" class="search" name="etime" value="" id="search">
				...
				<form>
					<table>
						...
						<?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr>
								...
								<td><a href="javascript:void(0)" attr-id="<?php echo ($user["id"]); ?>" id="delete" onclick="user.delete(<?php echo ($user["id"]); ?>)">删除</a></td>
			...
			<div class="rshow">
				<!-- <div class="searchdiv">
					<input type="text" class="search" value="" id="search">
